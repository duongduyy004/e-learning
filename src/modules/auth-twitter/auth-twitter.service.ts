...
export class AuthTwitterService {
    ...
    async login() {
        const clientId = this.configService.get('twitter.clientId', { infer: true })
        const clientSecret = this.configService.get('twitter.clientSecret', { infer: true })
        const callbackUrl = this.configService.get('twitter.callbackUrl', { infer: true })
        const consumerClient = new TwitterApi({ clientId, clientSecret });

        const link = consumerClient.generateOAuth2AuthLink(
            callbackUrl,
