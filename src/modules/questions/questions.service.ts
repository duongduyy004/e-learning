...
export class QuestionsService {
    constructor(
        ...
        @InjectRepository(QuestionChoiceEntity) private questionChoiceRepository: Repository<QuestionChoiceEntity>
    ) { }
    ...
    async updateQuestion(updateQUestionDto: UpdateQuestionDto[]) {
        ...
        for (const q of questions) {
            ...
            const choicesToDelete = q.choices.filter(
                choice => !incomingIds.has(choice.id) && choice.id !== correctChoice.id
            );


            deleteChoiceIds.push(...choicesToDelete.map(c => c.id));

            q.choices = currentQuestionChoices.map(item => (item.id !== correctChoice.id
                && {
                id: item.id,
    ...
    async getQuestions(categoryId: Category['id'], pagination: { limit: number, page: number }) {
